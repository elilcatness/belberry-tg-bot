...
def register_name(_, context: CallbackContext):
    ...
    context.bot.send_message(context.user_data['user'].id, 'Введите своё имя',
                             reply_markup=markup)
...
def register_phone(update: Update, context: CallbackContext):
    ...
    return context.bot.send_message(context.user_data['user'].id,
                                    'Укажите свой номер телефона', reply_markup=markup), 'register_phone'
