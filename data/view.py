...
class SpecialistViewPublic:
    @staticmethod
    @delete_last_message
    def show_all(_, context: CallbackContext, is_sub_already=False):
        if context.user_data.get('specialist_id'):
            context.user_data.pop('specialist_id')
        if not is_sub_already and context.user_data.get('found_prefix'):
            context.user_data.pop('found_prefix')
        ...
        context.user_data['spec_pages_count'] = build_pagination(
            ...
            ('специалист', 'специалиста', 'специалистов'), 'Услуги', 'services', is_sub_already)
    ...
    def show_all(_, context: CallbackContext, is_sub_already=False):
        with db_session.create_session() as session:
            if context.user_data.get('specialist_id'):
                ...
                if not context.user_data.get('found_prefix'):
                    context.user_data[
                        'found_prefix'] = f'. Специалист: <b>{spec.speciality} {spec.full_name}</b>'
