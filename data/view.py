...
class SpecialistViewPublic:
    @staticmethod
    @delete_last_message
    def show_all(_, context: CallbackContext, is_sub_already=False, prefix: str = None,
    ...
    def show_all(_, context: CallbackContext, is_sub_already: bool = False, prefix: str = '',
        ...
        with db_session.create_session() as session:
            if context.user_data.get('specialist_id'):
                ...
                if not context.user_data.get('found_suffix'):
                    context.user_data[
                        'found_suffix'] = f'. Специалист: <b>{spec.speciality} {spec.full_name}</b>'
