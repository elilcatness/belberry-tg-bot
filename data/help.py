...
def help_menu(_, context: CallbackContext):
    ...
    return context.bot.send_message(context.user_data['user'].id, 'Выберите опцию',
...
@delete_last_message
def ask_phone(_, context: CallbackContext):
...
def consult(update, context: CallbackContext):
    cfg = get_config()
